...
-   `gleu()`
    ...
    Scorer which computes a sentence-level `GLEU` score from n-gram overlap between model output and target references.

-   `comet()`

    Scorer which computes a COMET score for machine translation quality using source, candidate, and reference text.
