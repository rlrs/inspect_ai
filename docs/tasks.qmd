...
+-------------------+----------------+--------------------------------+
| `filters`         | None           | row filtering (pre-sample)     |
+-------------------+----------------+--------------------------------+
...
- `filters` are applied to raw dataset rows before mapping them to samples (so they can reference any source column). Supported operators: `eq`, `ne`, `in`, `not_in`, `gt`, `gte`, `lt`, `lte`, `between`, `is_null`, `not_null`, `exists`, `contains`, and `not_contains`.
- `filters` can be composed with `all`, `any`, and `not`. For example, include only rows where `include=true`:
